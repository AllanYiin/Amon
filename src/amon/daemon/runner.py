...
from amon.triggers import submit_job_run_request, submit_schedule_run_request
...
def _drain_event_queue(core: AmonCore, event_queue: deque[dict[str, Any]]) -> None:
    while event_queue:
        event = event_queue.popleft()
        trigger_results = _dispatch_trigger_run_requests(core, event)
        try:
            hook_results = process_event(
                event,
            ...
            log_event(
                {
                    ...
                    "actions": hook_results,
                    "trigger_runs": trigger_results,
                }
            )
        except Exception as exc:  # noqa: BLE001
            log_event(
...
def _dispatch_trigger_run_requests(core: AmonCore, event: dict[str, Any]) -> list[dict[str, Any]]:
    results: list[dict[str, Any]] = []
    if str(event.get("type") or "") == "schedule.fired":
        schedule_result = submit_schedule_run_request(event, data_dir=core.data_dir)
        if schedule_result is not None:
            results.append({"source": "schedule", **schedule_result})
    if str(event.get("scope") or "") == "job":
        job_result = submit_job_run_request(event, data_dir=core.data_dir)
        if job_result is not None:
            results.append({"source": "job", **job_result})
    return results
