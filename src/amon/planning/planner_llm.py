    ...
    *,
    ...
    if is_repair:
        return (
            ...
            "你只做任務拆解與圖定義修復；不要擅自對 agent/persona/assignment/指派做規劃。\n"
            "你只負責修正 TaskGraph v3 規劃結果，使其符合系統規格。\n\n"
            "可用 Skills 定義：\n"
            f"{skills_json}\n\n"
            "可用 Tools 定義：\n"
            f"{tools_json}\n\n"
            ...
            "- 要弄清楚 artifact（任務交付物）與 milestone（里程碑）都不應該作為任務節點定義；artifact 是任務節點輸出物，請寫入 taskSpec.artifacts，milestone 則不是 TASK。\n"
            "- 像「規格整理」可以是任務節點，其中可包含系統架構規劃書、API 規格書、使用介面草稿線圖等 artifacts；但「系統架構規劃書」本身不是 TASK，「規格交付給客戶」也不是 TASK，而是 milestone。\n"
            ...
            "- 每個 node.id 都必須是非空且唯一的字串；不可留空，也不可重複。\n"
            "- 任務節點的 node.title 不得為空，應撰寫長度不多於 10 個中文漢字的完整標題句，不可截斷。\n"
    ...
    return (
        ...
        "目標：在完整完成使用者委託的任務的前提下，將任務拆解成 TODO 子任務清單定義，並將此輸出可執行的 TaskGraph v3 圖定義；不要擅自對 agent/persona/assignment/指派做規劃。\n\n"
        "可用 Skills 定義：\n"
        f"{skills_json}\n\n"
        "可用 Tools 定義：\n"
        f"{tools_json}\n\n"
        ...
        "節點辨識與邊界（硬性）：\n"
        "- Task 是可由單一主執行者直接完成、且有明確 objective 與 definitionOfDone 的工作單元。\n"
        "- 要弄清楚 artifact（任務交付物）以及 milestone（里程碑）都不應該作為任務節點的定義；artifact 是任務節點的輸出物，請寫入 taskSpec.artifacts，milestone 不是 TASK。\n"
        "- 「規格整理」可以視為任務節點，其中包括系統架構規劃書、API 規格書、使用介面草稿線圖；但不應把「系統架構規劃書」或「規格交付給客戶」變成任務節點，前者是 artifact，後者是 milestone。\n"
        ...
        "- 問題拆解 / WBS / issue tree 類 skill 屬於 planner 內部能力；不要把這類 skill 放進任何 task 的 skillBindings。\n\n"
        "標題規則（硬性）：\n"
        "- 每個 node.id 都必須是非空且唯一的字串。\n"
        "- 任務節點的 node.title 不得為空。\n"
        "- 應撰寫長度不多於 10 個中文漢字的完整標題句，不可截斷。\n\n"
        "只允許的節點/邊/模式（enum）：\n"
        ...
        "- CONTROL/DEPENDS_ON 的方向固定是前置節點 -> 依賴它的節點，例如 concept_alignment -> design；只有真的需要分支時才使用 GATE + ROUTE。\n"
        "- 每個 TASK 至少要有 objective + task title + definitionOfDone（>=2 條）+ 主要 skillBindings（PRIMARY）。\n"
...
def _planner_user_prompt(
    *,
    ...
    sections = [
        ...
        "",
        "輸出結構：",
        "- 請只輸出一段 json code block。",
        "- 內容必須是可執行的 GraphDefinition JSON（純 JSON、無註解、雙引號）。",
        "- 不要輸出 Mermaid，也不要輸出任何 JSON 之外的補充文字。",
    ]
    if repair_error:
        sections.extend(
            [
...
def _coerce_node(raw: Any):
    ...
    if node_type == "TASK":
        ...
        return TaskNode(
            ...
            task_spec=task_spec_from_payload(spec_raw, node_id=node_id, task_title=title),
