...
class UIAsyncAPITests(unittest.TestCase):
    ...
    def test_project_context_stats_system_prompt_uses_default_when_config_empty(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            data_dir = Path(temp_dir) / "data"
            os.environ["AMON_HOME"] = str(data_dir)
            server = None
            try:
                core = AmonCore()
                core.initialize()
                project = core.create_project("context-system-prompt-default")
                ...
                self.assertEqual(response.status, 200)
                system_prompt = next(item for item in payload["categories"] if item.get("key") == "system_prompt")
                self.assertGreater(system_prompt.get("tokens", 0), 0)
                self.assertTrue(payload.get("meta", {}).get("has_system_prompt"))
            finally:
                if server:
                    server.shutdown()
                    server.server_close()
    ...
    def test_project_context_stats_skills_count_only_selected_or_command_invoked(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            data_dir = Path(temp_dir) / "data"
            os.environ["AMON_HOME"] = str(data_dir)
            server = None
            try:
                core = AmonCore()
                core.initialize()
                project = core.create_project("context-selected-skills")
                project_path = Path(project.path)

                skills_dir = Path(temp_dir) / "skills"
                (skills_dir / "enabled-skill").mkdir(parents=True, exist_ok=True)
                (skills_dir / "enabled-skill" / "SKILL.md").write_text("# enabled\n", encoding="utf-8")
                (skills_dir / "disabled-skill").mkdir(parents=True, exist_ok=True)
                (skills_dir / "disabled-skill" / "SKILL.md").write_text("# disabled\n", encoding="utf-8")
                core.set_config_value("skills.global_dir", str(skills_dir))
                core.scan_skills(project_path=project_path)
                core.set_config_value("skills.selected", ["enabled-skill"], project_path=project_path)

                sessions_dir = project_path / "sessions" / "chat"
                sessions_dir.mkdir(parents=True, exist_ok=True)
                (sessions_dir / "chat-01.jsonl").write_text(
                    json.dumps({"type": "user", "text": "繼續目前任務", "ts": "2026-01-01T10:00:00Z"}, ensure_ascii=False),
                    encoding="utf-8",
                )
                ...
                self.assertEqual(response.status, 200)
                skills = next(item for item in payload["categories"] if item.get("key") == "skills")
                self.assertEqual(skills.get("items"), 1)
            finally:
                if server:
                    server.shutdown()
                    server.server_close()
