...
class UIShellSmokeTests(unittest.TestCase):
    ...
    def test_chat_stream_uses_view_renderers_and_orchestrator(self) -> None:
        ...
        self.assertIn('messageRenderer.applyTokenChunk(data.text || "", resolveNodeMeta(data));', chat_js)
        self.assertIn("applySessionFromEvent(data)", chat_js)
        self.assertIn("messageRenderer.upsertToolCall(nodeMeta, {", chat_js)
        ...
        self.assertIn("chat-msg__node-label", message_renderer_js)
        self.assertIn("chat-msg__tools", message_renderer_js)
        self.assertIn("function upsertToolCall(meta = {}, toolMeta = {})", message_renderer_js)
