...
### 5.4 Chat route / `thread_id` 來源（hash route 解析）
...
## 9) Phase 4：Context clear(chat) 帶 thread_id 驗收步驟

1. 進入 `#/context` 並確認已選 project。
2. 檢查「清空本次對話」按鈕狀態：
   - 若目前沒有 `thread_id`：按鈕應 disabled，提示需先有聊天 session。
   - 若已有 `thread_id`：按鈕可操作。
3. 送出 clear(chat)（帶 `thread_id`）：
   - 預期 request body 含 `scope=chat, project_id, thread_id`。
   - 預期只清除該 `thread_id` 對話資料，不影響同專案其他 chat。
4. 送出 clear(chat) 且缺 `thread_id`：
